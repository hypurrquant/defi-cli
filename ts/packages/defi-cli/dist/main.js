...
var init_dist = __esm({
  "../defi-core/dist/index.js"() {
    ...
    InterestRateMode = /* @__PURE__ */ ((InterestRateMode2) => {
    ...
    UniswapV3Adapter = class {
      ...
      async quote(params) {
        ...
        try {
          ...
          const errMsg2 = String(e);
          if (errMsg2.includes("STF") || errMsg2.includes("insufficient")) {
            throw DefiError.unsupported(
          ...
          throw DefiError.rpcError(`[${this.protocolName}] swap simulation for quote failed: ${errMsg2}`);
        }
...
`
  ...
  async simulate(tx) {
    ...
    try {
      ...
      const errMsg2 = String(e);
      const revertReason = extractRevertReason(errMsg2);
      return {
        tx_hash: void 0,
...
var ERC20_DECIMALS_ABI2 = parseAbi30(["function decimals() external view returns (uint8)"]);
    ...
    for (const p of active) {
      ...
      if (rewardToken) {
        if (rewardToken.toLowerCase() === WRAPPED_NATIVE.toLowerCase()) {
          ...
          if (tokenPoolAddr) {
            try {
              const [resRaw] = await multicallRead(rpcUrl, [
                [tokenPoolAddr, encodeFunctionData27({ abi: V2_PAIR_ABI, functionName: "getReserves" })]
              ]);
              if (resRaw) {
                const _resAbi = parseAbi30(["function f() view returns (uint112, uint112, uint32)"]);
                const [r0, r1] = decodeFunctionResult8({ abi: _resAbi, functionName: "f", data: resRaw });
      ...
      try {
        const [resRaw, tsRaw] = await multicallRead(rpcUrl, [
          [p.pool, encodeFunctionData27({ abi: V2_PAIR_ABI, functionName: "getReserves" })],
          [p.pool, encodeFunctionData27({ abi: V2_PAIR_ABI, functionName: "totalSupply" })]
        ]);
        if (resRaw && tsRaw) {
          const _resAbi = parseAbi30(["function f() view returns (uint112, uint112, uint32)"]);
          const [r0, r1] = decodeFunctionResult8({ abi: _resAbi, functionName: "f", data: resRaw });
...
function registerLP(parent, getOpts, makeExecutor2) {
            ...
            if (adapter.discoverGaugedPools) {
              ...
              for (const p of pools) {
                results.push({
                  ...
                  type: "EMISSION",
                  source: "gauge",
                  rewardRate: p.rewardRate ? p.rewardRate.toString() : void 0,
                  totalStaked: p.totalStaked ? p.totalStaked.toString() : void 0,
  ...
  lp.command("claim").description("Claim rewards from a pool (fee or emission)").requiredOption("--protocol <protocol>", "Protocol slug").option("--pool <address>", "Pool address (required for farming/LB)").option("--gauge <address>", "Gauge contract address (required for solidly/hybra)").option("--token-id <id>", "NFT tokenId (for CL gauge or farming positions)").option("--bins <binIds>", "Comma-separated bin IDs (for Merchant Moe LB)").option("--address <address>", "Wallet address (defaults to DEFI_WALLET_ADDRESS)").action(async (opts) => {
    ...
    if (["solidly_v2", "solidly_cl", "algebra_v3", "hybra"].includes(iface) || iface === "uniswap_v3" && protocol.contracts?.["voter"]) {
      ...
      whitelist.map(async (entry) => {
        try {
          ...
          if (entry.type === "gauge" && entry.pool) {
            ...
            const proto = protos[0];
            if (["solidly_v2", "solidly_cl", "algebra_v3", "hybra"].includes(proto.interface) || proto.interface === "uniswap_v3" && proto.contracts?.["voter"]) {
              const chainTokens = registry.tokens.get(chainName)?.map((t) => t.address);
...
// src/utils.ts
init_dist();
function requireChain(parent, getOpts) {
  const chain = parent.opts().chain;
  if (!chain) {
    printOutput({ error: "--chain is required (e.g. --chain hyperevm)" }, getOpts());
    return null;
  }
...
function resolveContext(parent, getOpts, protocolSlug) {
  const chainName = requireChain(parent, getOpts);
  if (!chainName) return null;
  const registry = Registry.loadEmbedded();
  const chain = registry.getChain(chainName);
  const protocol = protocolSlug ? registry.getProtocol(protocolSlug) : void 0;
  return { chainName, registry, chain, rpcUrl: chain.effectiveRpcUrl(), protocol };
}
function resolveTokenAddress(registry, chainName, tokenOrAddress) {
  if (/^0x[0-9a-fA-F]{40}$/.test(tokenOrAddress)) return tokenOrAddress;
...
var ORACLE_ABI4 = parseAbi31([
  "function getAssetPrice(address asset) external view returns (uint256)"
]);
var POOL_ABI3 = parseAbi31([
  "function getUserAccountData(address user) external view returns (uint256 totalCollateralBase, uint256 totalDebtBase, uint256 availableBorrowsBase, uint256 currentLiquidationThreshold, uint256 ltv, uint256 healthFactor)"
]);
...
async function takeSnapshot(chainName, wallet, registry) {
  ...
  for (const t of registry.tokens.get(chainName) ?? []) {
    ...
    calls.push([
      entry.address,
      encodeFunctionData28({ abi: ERC20_ABI4, functionName: "balanceOf", args: [user] })
    ]);
  ...
  for (const p of lendingProtocols) {
    calls.push([
      p.contracts["pool"],
      encodeFunctionData28({ abi: POOL_ABI3, functionName: "getUserAccountData", args: [user] })
    ]);
...
// src/commands/portfolio.ts
var ERC20_ABI5 = parseAbi33([
  "function balanceOf(address owner) external view returns (uint256)"
]);
var POOL_ABI4 = parseAbi33([
  "function getUserAccountData(address user) external view returns (uint256 totalCollateralBase, uint256 totalDebtBase, uint256 availableBorrowsBase, uint256 currentLiquidationThreshold, uint256 ltv, uint256 healthFactor)"
]);
var ORACLE_ABI5 = parseAbi33([
  "function getAssetPrice(address asset) external view returns (uint256)"
]);
...
function registerPortfolio(parent, getOpts) {
  ...
  portfolio.command("show").description("Show current portfolio positions").requiredOption("--address <address>", "Wallet address to query").action(async (opts) => {
    ...
    for (const p of lendingProtocols) {
      calls.push([
        p.contracts["pool"],
        encodeFunctionData29({ abi: POOL_ABI4, functionName: "getUserAccountData", args: [user] })
      ]);
      callLabels.push(`lending:${p.name}`);
    }
    const oracleEntry = registry.getProtocolsForChain(chainName).find((p) => p.interface === "aave_v3" && p.contracts?.["oracle"]);
