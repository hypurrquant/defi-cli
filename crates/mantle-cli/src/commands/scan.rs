...
const STABLECOINS: &[&str] = &["USDC", "USDT", "DAI", "USDT0", "USDe"];
...
pub async fn run(
    ...
    loop {
        ...
        if calls.is_empty() {
            return Err(DefiError::Unsupported(
                "No scannable resources on Mantle".into(),
            ));
        }
                        ...
                        if let Some(&prev) = prev_rates.get(&key) {
                            ...
                            if change > args.rate_threshold {
                                ...
                                alerts.push(serde_json::json!({
                                    "pattern": "exchange_rate_anomaly", "severity": severity,
                                    "protocol": protocol, "vtoken": vtoken,
                                    "prev_rate": round6(prev), "curr_rate": round6(rate),
                                    "change_pct": round2(change),
                                    "action": format!("possible donation attack on {} {}", protocol, vtoken),
                                }));
                            }
                        }
            ...
            if !all_below {
                for (asset, pair, price) in &stable_prices {
                    if *price < args.stable_threshold {
                        ...
                        alerts.push(serde_json::json!({
                            "pattern": "stablecoin_depeg", "severity": severity,
                            "asset": asset, "pair": pair, "price": round4(*price),
                ...
                if let Some(&dex_price) = dex_by_token.get(token) {
                    for (oracle_name, oracle_price) in oracle_entries {
                        ...
                        if deviation > args.oracle_threshold {
                            ...
                            alerts.push(serde_json::json!({
                                "pattern": "oracle_divergence", "severity": severity,
                                "asset": token, "oracle": oracle_name,
                                "oracle_price": round4(*oracle_price), "dex_price": round4(dex_price),
                                "deviation_pct": round2(deviation), "action": action,
                            }));
                        }
                    }
                }
        ...
        let result = serde_json::json!({
            "timestamp": timestamp, "chain": "Mantle",
            "scan_duration_ms": scan_ms, "patterns": &args.patterns,
            "alert_count": alerts.len(), "alerts": alerts, "data": data,
        });
